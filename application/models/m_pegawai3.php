...
class M_pegawai3 extends CI_Model {
    ...
    var $order = array('d.Lokasi' => 'desc'); // default order 
    ...
    private function _get_datatables_query($bulan, $tahun) {
        ...
        $this->db->join('tblstruktural b', 'a.unit_kerja=b.kodeunit', 'LEFT');
        $this->db->join("tblrekaptk c", 'c.nip=a.nip', 'LEFT');
        $this->db->join("tbllokasikerja d", 'c.kodelokasi=d.KodeLokasi', 'LEFT');
        $this->db->join("opmt_bulanan_skp e", "e.id_dd_user=a.id_dd_user AND coalesce(nilai_skp,0)=0 AND e.bulan={$bulan} AND e.tahun={$tahun}", 'INNER');
        $i = 0;
