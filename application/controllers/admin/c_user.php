...
//output to json format
    ...
    public function ubah($id) {
        $x['dt_user'] = $this->db->query("SELECT a.id_dd_user,a.nama,a.nip,a.username,a.password,b.KodeGol,concat(b.Golongan,'-',b.Pangkat)nama_golongan,
c.kodeunit,c.unitkerja,d.kodejab,d.jabatan            
FROM dd_user a 
LEFT JOIN tblgolongan b ON a.gol_ruang=b.KodeGol
...
WHERE a.id_dd_user={$id}")->result_array();
    ...
    public function gol_ruang_json() {
        $nama = $this->input->post('q');
        $q = "SELECT KodeGol as id, concat(Golongan, '-', Pangkat) as label, concat(Golongan, '-', Pangkat) as value FROM tblgolongan WHERE (Golongan LIKE '%" . $nama . "%' OR Pangkat LIKE '%" . $nama . "%')  ORDER BY value ASC";
        $data = $this->db->query($q)->result();
        echo json_encode($data);
    }
    ...
    public function jabatan_json() {
        $nama = $this->input->post('q');
        $q = "SELECT kodejab as id, concat(kodejab, ' - ', jabatan) as label, concat(kodejab, ' - ', jabatan) as value FROM tbljabatan WHERE jabatan LIKE '%" . $nama . "%'  ORDER BY value ASC";
        $data = $this->db->query($q)->result();
        echo json_encode($data);
    }
